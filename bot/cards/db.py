...
def get_random_template_for_mafia():
    ...
    mafia_weights = {"B": 0.50, "A": 0.35, "S": 0.15}
...
def get_template_counts_by_rarity():
    conn = Connection()
    cur = conn.execute(q("SELECT rarity, COUNT(*) as count FROM card_templates GROUP BY rarity ORDER BY CASE rarity WHEN 'S' THEN 0 WHEN 'A' THEN 1 WHEN 'B' THEN 2 WHEN 'C' THEN 3 WHEN 'D' THEN 4 WHEN 'F' THEN 5 ELSE 6 END"))
    col_keys = [d[0] for d in cur.description] if USE_PG else None
    rows = cur.fetchall()
    conn.close()
    if USE_PG:
...
def get_card_counts_by_rarity():
    ...
    rows = cur.fetchall()
    if USE_PG:
        result = {dict(zip(col_keys, r))["rarity"]: dict(zip(col_keys, r))["count"] for r in rows}
    else:
        result = {r[0]: r[1] for r in rows}
    # Also get raw total row count for discrepancy detection
    cur2 = conn.execute("SELECT COUNT(*) FROM card_instances")
    raw_total = cur2.fetchone()[0]
    conn.close()
